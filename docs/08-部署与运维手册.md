...
> 运行端口：容器内 `3000`，宿主机建议仅回环映射（常见 `127.0.0.1:8000:3000`）
...
| `AUTH_RATE_LIMIT_TRUST_PROXY_HEADERS` | 按需 | 受控反向代理场景设为 `1`，让登录限流按 `x-forwarded-for` 等真实客户端 IP 生效 |
...
5. 发布后最小回归（健康检查 + API 冒烟 + 登录限流）：

```bash
RELEASE_VERIFY_BASE_URL=http://127.0.0.1:8000 npm --workspace apps/ecs run verify:release
```

注意：该脚本会写入 smoke 测试数据（课堂、作业、讨论等），建议只在预发/受控生产窗口执行。

## 4. ESA Pages 部署（静态裁剪版）
...
2. ESA Pages GitHub 构建配置：

- Root Dir：`apps/esa`
- 输出目录：`out/`

说明：ESA 版为纯静态导出，不依赖源站。
...
```bash
npm --workspace apps/ecs run backup:create
```

恢复：

```bash
npm --workspace apps/ecs run backup:restore -- data/backups/<backup-file>.db
```
